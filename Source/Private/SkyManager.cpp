...
FRotator ASkyManager::CalculateSunMoonAngle(float Latitude2, float Longitude2, float TimeZone, bool bIsDaylightSavingTime, int32 Year, int32 Month, int32 Day, int32 Hours, int32 Minutes, int32 Seconds)
{
	...
	InitSunMoonCalculator(Year, Month, Day, Hours, Minutes, Seconds, Longitude2 * DEG_TO_RAD, Latitude2 * DEG_TO_RAD, TimeZone);
	calcSunAndMoon();
...
FEphemeris ASkyManager::CalculateMoonAngle(float Latitude2, float Longitude2, float TimeZone, bool bIsDaylightSavingTime,
...
FEphemeris ASkyManager::CalculateSunAngle(float Latitude2, float Longitude2, float TimeZone, bool bIsDaylightSavingTime, int32 Year, int32 Month, int32 Day, int32 Hours, int32 Minutes, int32 Seconds)
{
	InitSunMoonCalculator(Year, Month, Day, Hours, Minutes, Seconds, Longitude2 * DEG_TO_RAD, Latitude2 * DEG_TO_RAD, TimeZone);
	calcSunAndMoon();
...
/**
	 ...
	 */
void ASkyManager::InitSunMoonCalculator(int year, int month, int day, int h, int m, int s, double obsLonIn, double obsLatIn, float TimeZone) {
...
TArray<double>  ASkyManager::getSun() {
	double L = 0.0, R = 0.0;
	double t2 = t * 0.01;
	//DOUBLE CHECK THIS!!
...
TArray<double> ASkyManager::getMoon() {
...
FEphemeris* ASkyManager::doCalc(TArray<double> pos, bool geocentric) {
 ...
 */
TArray<int32>  ASkyManager::getDate(double jd) {
	if (jd < 2299160.0 && jd >= 2299150.0)
 ...
 * @throws Exception If the date does not exists.
 */
FString  ASkyManager::getDateAsString(double jd) {
 ...
 */
double  ASkyManager::normalizeRadians(double r)
{
...
double  ASkyManager::obtainAccurateRiseSetTransit(double riseSetJD, EVENT index, int niter, bool sunIn) {
	double step = -1;
...
TArray<double> ASkyManager::getMoonDiskOrientationAngles()
{
	sun = doCalc(getSun(), false);
